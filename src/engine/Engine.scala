...
class TurnBasedEngine[V <: Vertex with Action[V], E <: Edge[V]](graph: Graph[V with Action[V], E],count:Int) extends Engine{
  override def run={
...
class RoundBasedEngine[V <: Vertex with Transactions, E <: Edge[V]](graph: Graph[V with Transactions, E],count:Int) extends Engine{
	override def run={
	  for (_ <- 0 until count)
...
class EventBasedEngine[V <: Vertex, E <: Edge[V]](graph: Graph[V, E],count:Int) extends Engine{

...
/**
 * Dit zal voor morgen of mss vanavond laat zijn want ik heb honger! 
 * Deze dingen zou ik nodig hebben :)
 */

class ROutputStatistics {
...
//        re.eval( fm2 <- lm(y ~ x1 , data = b))
//
//  
//  calculateClusterOfVerticeswith2attr((attr1), (attr2))
//  	
//  	mydata = graph.traverser.(vertex.id, valueofattr(attr1), valueofattr(attr2) 
// 
